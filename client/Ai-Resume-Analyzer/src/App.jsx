    ...
    try {
      ...
      setError(
        err.response?.data?.error ||
          "We encountered an issue analyzing your resume. Please try again.",
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="px-8 py-12 sm:px-12 sm:py-16 bg-gradient-to-r from-blue-600 via-indigo-600 to-violet-600 text-white relative overflow-hidden shrink-0">
          ...
          <div className="relative z-10 flex flex-col items-center text-center">
            ...
            <p className="text-blue-100/90 text-lg sm:text-xl max-w-2xl mx-auto font-medium leading-relaxed">
              Unlock your career potential. Upload your PDF or paste your resume
              below to get instant AI insights.
            </p>
          </div>
        </div>
          ...
          <div className="space-y-6 flex-grow">
            ...
            <div className="flex p-1 space-x-1 bg-slate-100/80 rounded-xl w-full sm:w-fit mx-auto border border-slate-200/60 shadow-inner mb-8">
              <button
                ...
                className={`flex-1 sm:flex-none flex items-center justify-center space-x-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all duration-300 ${
                  mode === "text"
            ...
            <div className="relative group">
              ...
              {mode === "text" ? (
                ...
                <div
                  {...getRootProps()}
                  className={`relative min-h-[320px] rounded-2xl border-2 border-dashed flex flex-col items-center justify-center p-8 transition-all duration-300 cursor-pointer overflow-hidden ${
                    isDragActive
                ...
                >
                  ...
                  {pdfFile ? (
                    ...
                    <div className="flex flex-col items-center text-center space-y-4 z-10">
                      <div
                        className={`p-4 rounded-full shadow-sm transition-colors duration-300 ${isDragActive ? "bg-blue-100" : "bg-white"}`}
                      >
                        <UploadCloud
                          className={`w-12 h-12 ${isDragActive ? "text-blue-600" : "text-slate-400"}`}
                        />
                      </div>
                      <div>
            ...
            <div className="flex justify-end pt-4">
              <button
                ...
                disabled={
                  loading ||
                  (mode === "text" && !resume.trim()) ||
                  (mode === "pdf" && !pdfFile)
                }
                className={`relative overflow-hidden group px-8 sm:px-10 py-4 rounded-xl text-lg font-bold transition-all duration-300 flex items-center justify-center space-x-3 w-full sm:w-auto ${
                  loading ||
                  (mode === "text" && !resume.trim()) ||
                  (mode === "pdf" && !pdfFile)
                    ? "bg-slate-200 text-slate-400 cursor-not-allowed shadow-none"
                    : "bg-blue-600 text-white shadow-[0_8px_30px_rgba(37,99,235,0.25)] hover:shadow-[0_8px_30px_rgba(37,99,235,0.4)] hover:-translate-y-1 active:translate-y-0"
                }`}
              >
                {!loading &&
                  ((mode === "text" && resume.trim()) ||
                ...
                <span className="relative z-10 flex items-center space-x-2">
                  {loading ? (
                    <>
                      ...
                      <span>
                        Analyzing... (may take up to 30s on first load)
                      </span>
                    </>
                  ) : (
                    <>
